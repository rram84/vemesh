...
# Sriramajayam
#
# Performance plots: embedded shapes. Two figures from the staged data:
#
# 1) DISTRIBUTION (shapes_condition_dist): grouped VIOLIN plots with the shapes along
#    the x-axis and, within each shape, four violins -- one per variant -- showing the
#    spread of conditioning over the random realizations. A dark dot marks the median,
#    a thick bar the IQR. The KDE is computed in log10(condition number) space and the
#    y-axis is linear in log10 with powers-of-ten tick labels, so the density is not
#    warped by a log axis. (Unlike the circle case there is no refinement sweep, so
#    the condition number is plotted directly -- no h-scaling.)
#
# 2) CORRELATION (shapes_correlation_minq): does element-level optimization show up in
#    assembled-level conditioning? Scatter pooled over every shape x realization x
#    variant of
...
#    growth of the bare-stiffness condition number, so the cloud is dimensionless.
#
# Data sources (no MATLAB, no re-run):
#   - ratio (lambda_max/lambda_2) from analyze_shapes.sh's per-shape CSV;
...
#     likewise carries it because run_shapes.sh stages vemesh_app's input_mesh.vtk
#     (which embeds face_quality). Any mesh lacking it is silently skipped (counted).
#
# Both figures read whatever per-shape CSVs exist, so they can run while
...
DX = 0.17          # spacing between adjacent violins within a shape group
...
# --------------------------------------------------------------------------- #
# Discover shapes from the per-shape CSVs (shared by both figures)
# --------------------------------------------------------------------------- #
if not os.path.isdir(OUT):
    die(f"no output/shapes at {OUT} (run analyze_shapes.sh first)")
...
if not shapes:
    die(f"no per-shape CSVs under {OUT} (run analyze_shapes.sh first)")


# =========================================================================== #
# Figure 1: distribution (violins of condition number per shape)
# =========================================================================== #
def make_distribution_figure():
    data = {}            # shape -> variant -> log10(ratio) array
    for shape in shapes:
        vals = {v: [] for v in VARIANTS}
        with open(os.path.join(OUT, f"{shape}.csv"), newline="") as fh:
        ...
        data[shape] = {v: np.asarray(a) for v, a in vals.items() if len(a) >= 2}

    if not any(data[s] for s in shapes):
        sys.stderr.write("warn: no usable rows for the distribution figure; skipping\n")
        return
    ...
    all_vals = np.concatenate([a for s in shapes for a in data[s].values()])
    ...
    ax.set_xticks(range(1, len(shapes) + 1))
    ax.set_xticklabels(shapes)
    ax.set_xlabel("shape")
    ax.set_ylabel(r"condition number  ($\lambda_{\max}/\lambda_{\min}$)")
    ax.set_title("embedded shapes: distribution of conditioning over random realizations")
...
def make_correlation_figure():
        ...
        with open(os.path.join(OUT, f"{shape}.csv"), newline="") as fh:
            ...
            for row in reader:
                ...
                if not (ratio > 0 and math.isfinite(ratio)):
                    continue
                vtk = os.path.join(OUT, shape, f"{realn}__{variant}.vtk")
    ...
    if total == 0:
        sys.stderr.write("warn: no (quality, conditioning) pairs; skipping correlation figure "
                         "(re-run run_shapes.sh / check face_quality)\n")
        return
    if nmiss:
    ...
    if bx.size >= 2:
        ...
        sys.stderr.write("note: <2 baseline points; drawing the cloud without a fit line "
                         "(re-run run_shapes.sh so baseline carries face_quality)\n")
    ...
    ax.set_title("embedded shapes: scale-corrected condition number vs worst element quality")
