...
namespace vm
{
  ...
  bool is_point_feasible(const boost_box_t& bbox,
			 ...
			 const boost_point_t& sample)
  {
    ...
    for(auto& Y:connectedVertices)
      {
	boost_segment_t seg(sample, boost_point_t(EPS*bg::get<0>(sample)+(1.-EPS)*Y[0],
						  EPS*bg::get<1>(sample)+(1.-EPS)*Y[1])); // boundary vertex moved inward
	if(bg::within(poly, seg)==false)
	  return false;
      }
				    ...
				    const int              num_edge_samples)        // number of samples to generate per edge  
  {
    ...
    std::vector<pmp::Vertex> connected_vertices{};
    ...
    std::uniform_real_distribution<> ydis(bf::get<1>(min_corner), bg::get<1>(max_corner));
      ...
      for(int iter=0; iter<num_edge_samples; ++iter)
	{
	  ...
	  if(is_point_feasible(poly, connectedVertices, sample)==true)
	    feasible_points.push_back({bg::get<0>(sample), bg::get<1>(sample)});
	}
