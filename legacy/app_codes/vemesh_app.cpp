...
int main(int argc, char **argv)
{
  ...
  for(int iter=0; iter<cfg.num_iters; ++iter) {
    ...
    if(cfg.output_mode == CLIConfig::OutputMode::IterationEnd)
      {
...
CLIConfig parse_cli(int argc, char** argv)
{
  ...
  // Optimization mode
  if (*opt_a)       cfg.mode = Mode::Agglomerate;
  else if (*opt_r)  cfg.mode = Mode::Relax;
  else if (*opt_ar) cfg.mode = Mode::AgglomerateRelax;
  else              cfg.mode = Mode::RelaxAgglomerate;
...
// Generate a callback function
ProgressCallback make_mesh_callback(int iter,
                                    const fs::path &outpath,
                                    CLIConfig::MeshOutputMode output_mode,
				    const std::string descr)
{
  ...
  if(output_mode == CLIConfig::MeshOutputMode::None || output_mode==CLIConfig::MeshOutputMode::IterationEnd)
    return [](const ProgressInfo&, const pmp::SurfaceMesh&, const MeshOptimizer&) { return true; };
  
  // Output mode is detailed. generate a callback that writes VTK files
  return [iter, outpath, descr](const ProgressInfo& info,
