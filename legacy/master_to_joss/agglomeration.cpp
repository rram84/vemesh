...
int main()
{
  ...
  for(auto f:f_iterator)
    if(!mesh.is_deleted(f) && mesh.is_valid(f))
      if(vm::MeshFaceQuality_f(mesh,f,qface)<qeps)
  ...
  for(auto f:f_iterator_post)
    q_post.insert({f.idx(), vm::MeshFaceQuality_f(mesh, f, qface)});
