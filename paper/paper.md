...
- **Quality metric specification**: The metric is evaluated through a function that takes only nodal coordinates of a face as input. In particular, the metric has no context of the mesh, neighboring elements, or any state. This makes it trivial for users to implement a custom metric and safe to invoke concurrently. This feature makes the library metric-agnostic rather than VEM specific; the element stability ratio and the shape ratio provided in the library are two specific instances. 
